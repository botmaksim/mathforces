...
ArchiveTab::ArchiveTab(const QString& token, QWidget* parent) : QWidget(parent), m_token(token) {
    ...
    connect(r, &QNetworkReply::finished, [this, r, m]() {
        if (r->error() == QNetworkReply::NoError) {
            ...
            for (int i = 0; i < arr.size(); ++i) {
                ...
                m_table->setItem(i, 0, new QTableWidgetItem(QString::number(o["id"].toInt())));
                m_table->setItem(i, 1, new QTableWidgetItem(o["title"].toString()));
                m_table->setItem(i, 2, new QTableWidgetItem(o["tags"].toString()));
                m_table->setItem(i, 3, new QTableWidgetItem(QString::number(o["difficulty"].toInt())));
            }
        }
...
void ArchiveTab::openTask(int taskId) {
    ...
    QMessageBox::information(this, "Задача", "Открытие задачи ID: " + QString::number(taskId));
}
