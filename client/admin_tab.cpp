...
AdminTab::AdminTab(const QString& token, QWidget* parent) : QWidget(parent), m_token(token) {
    ...
    l1->addWidget(new QLabel("Название:")); l1->addWidget(m_cTitle); 
    l1->addWidget(new QLabel("Начало:")); l1->addWidget(m_cStart); 
    l1->addWidget(new QLabel("Продолжительность:")); l1->addWidget(m_cDuration); 
    l1->addWidget(new QLabel("Описание:")); l1->addWidget(m_cDesc); 
    ...
    m_tTags = new QLineEdit(); m_tTags->setPlaceholderText("Теги (через запятую)...");
    m_tDifficulty = new QLineEdit(); m_tDifficulty->setPlaceholderText("Сложность (например 1200)");
    
    QPushButton* btnPreviewEditorial = new QPushButton("Предпросмотр разбора (Typst)", this);
    ...
    l2->addWidget(m_tTags); l2->addWidget(m_tDifficulty);
    l2->addWidget(new QLabel("Условие:")); l2->addWidget(m_tDesc);
    l2->addWidget(m_tCorrectAnswer);
    l2->addWidget(new QLabel("Авторское решение:")); l2->addWidget(m_tEditorial); l2->addWidget(btnPreviewEditorial); l2->addWidget(m_tSendEditorialToAi);
    ...
    ML->addWidget(g1); ML->addWidget(g2); ML->addWidget(g3);
    ...
    connect(r, &QNetworkReply::finished, [this, r, m]() {
        if (r->error() == QNetworkReply::NoError) {
            ...
            for (auto v : arr) {
                ...
                m_tContestId->addItem(QString("%1 (ID: %2)").arg(o["title"].toString()).arg(o["id"].toInt()), o["id"].toInt());
            }
        }
        r->deleteLater(); m->deleteLater();
    });
...
void AdminTab::createTask() {
    qDebug() << "Client: Creating task:" << m_tTitle->text() << "for contest ID:" << m_tContestId->currentData().toInt();
    QNetworkAccessManager* m = new QNetworkAccessManager(this);
