...
void ApiClient::fetchArchiveTasks(const QString& token, const QString& tags, const QString& minDiff, const QString& maxDiff) {
...
void ApiClient::searchUsers(const QString& token, const QString& query) {
    QNetworkRequest req(QUrl(QString(ApiConfig::baseUrl + "/api/users/search?q=%1").arg(query)));
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QNetworkReply* reply = m_manager->get(req);
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
...
void ApiClient::addFriend(const QString& token, int friendId) {
    ...
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["friend_id"] = friendId;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit friendAdded();
        else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}
...
void ApiClient::removeFriend(const QString& token, int friendId) {
    ...
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit friendRemoved();
        else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}
...
void ApiClient::requestCode(const QString& email) {
    ...
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    QJsonObject j; j["email"] = email;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit codeRequested();
        } else emit errorOccurred("Ошибка при запросе кода (возможно email занят / не существует)");
        reply->deleteLater();
    });
}

void ApiClient::registerUser(const QString& code, const QString& email, const QString& username, const QString& name, const QString& password) {
    ...
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        ...
        } else emit errorOccurred("Регистрация не удалась (неверный код?): " + reply->errorString());
        reply->deleteLater();
    });
}
...
void ApiClient::fetchUsers(const QString& token) {
    ...
    QNetworkReply* reply = m_manager->get(req);
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit usersLoaded(QJsonDocument::fromJson(reply->readAll()).array());
        else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}

void ApiClient::changeUserRole(const QString& token, int userId, const QString& role) {
    QNetworkRequest req(QUrl(ApiConfig::baseUrl + "/api/users/role"));
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["user_id"] = userId; j["role"] = role;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit userUpdated();
        else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}
...
void ApiClient::changeUserBan(const QString& token, int userId, bool isBanned) {
    ...
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["user_id"] = userId; j["is_banned"] = isBanned;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit userUpdated();
        else emit errorOccurred(reply->errorString());
...
void ApiClient::changeUserBlog(const QString& token, int userId, bool canBlog) {
    ...
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["user_id"] = userId; j["can_blog"] = canBlog;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit userUpdated();
        else emit errorOccurred(reply->errorString());
...
void ApiClient::fetchResults(int contestId) {
    QNetworkRequest req(QUrl(QString(ApiConfig::baseUrl + "/api/results?contest_id=%1").arg(contestId)));
    QNetworkReply* reply = m_manager->get(req);
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
...
void ApiClient::rateContest(const QString& token, int contestId) {
    ...
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["contest_id"] = contestId;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit contestRated();
        else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}
...
void ApiClient::createDraftContest(const QString& token) {
    ...
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit draftCreated();
        else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}

void ApiClient::updateContest(const QString& token, int contestId, const QString& title, const QString& start, double duration, const QString& desc, bool isPublished) {
    QNetworkRequest req(QUrl(ApiConfig::baseUrl + "/api/admin/contest"));
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j;
    ...
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit contestUpdated();
        else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}

void ApiClient::createTask(const QString& token, int contestId, const QString& title, int maxScore, int maxSubmissions, const QString& desc, const QString& type, const QString& correctAnswer, const QString& editorial, bool sendEditorial, const QString& aiComment, const QString& tags, int difficulty) {
    ...
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) emit taskCreated();
        else emit errorOccurred(reply->errorString() + "-" + reply->readAll());
        reply->deleteLater();
    });
}
...
void ApiClient::compileTypst(const QString& code, bool realtime) {
    ...
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    QJsonObject j; j["code"] = code;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply, realtime]() {
        if (reply->error() == QNetworkReply::NoError) {
            if (realtime) emit realtimeTypstCompiled(reply->readAll());
            else emit typstCompiled(reply->readAll());
        } else {
            if (!realtime) emit errorOccurred(reply->errorString() + "\n" + reply->readAll());
        }
        reply->deleteLater();
    });
}
...
void ApiClient::fetchContestTasks(const QString& token, int contestId) {
    QNetworkRequest req(QUrl(QString(ApiConfig::baseUrl + "/api/tasks?contest_id=%1").arg(contestId)));
    QNetworkReply* reply = m_manager->get(req);
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit contestTasksLoaded(QJsonDocument::fromJson(reply->readAll()).array());
        } else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}

void ApiClient::submitAnswer(const QString& token, int taskId, const QString& answer) {
    QNetworkRequest req(QUrl(ApiConfig::baseUrl + "/api/submit"));
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["task_id"] = taskId; j["answer"] = answer;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
...
void ApiClient::fetchMySubmissions(const QString& token, int taskId) {
    ...
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit mySubmissionsLoaded(QJsonDocument::fromJson(reply->readAll()).array());
        } else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}
...
void ApiClient::fetchAllSubmissions(const QString& token, int taskId) {
    QNetworkRequest req(QUrl(QString(ApiConfig::baseUrl + "/api/submissions/all?task_id=%1").arg(taskId)));
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QNetworkReply* reply = m_manager->get(req);
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit allSubmissionsLoaded(QJsonDocument::fromJson(reply->readAll()).array());
        } else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}

void ApiClient::submitHack(const QString& token, int submissionId, const QString& hackText) {
    QNetworkRequest req(QUrl(ApiConfig::baseUrl + "/api/hacks"));
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["submission_id"] = submissionId; j["hack_text"] = hackText;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit hackSuccessful();
...
void ApiClient::fetchProfile(const QString& token, int targetUserId) {
    ...
    if (targetUserId != -1) url = QUrl(QString(ApiConfig::baseUrl + "/api/users/profile?id=%1").arg(targetUserId));
    QNetworkRequest req(url);
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QNetworkReply* reply = m_manager->get(req);
    connect(reply, &QNetworkReply::finished, [this, reply, targetUserId]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit profileLoaded(QJsonDocument::fromJson(reply->readAll()).object(), targetUserId);
        } else emit errorOccurred(reply->errorString());
        reply->deleteLater();
    });
}
...
void ApiClient::fetchBlogPosts(const QString& token, int userId) {
    QNetworkRequest req(QUrl(QString(ApiConfig::baseUrl + "/api/blog/posts?user_id=%1").arg(userId)));
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QNetworkReply* reply = m_manager->get(req);
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
...
void ApiClient::addBlogPost(const QString& token, const QString& content) {
    ...
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["content"] = content;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit blogPostAdded();
...
void ApiClient::addComment(const QString& token, int postId, const QString& content) {
    QNetworkRequest req(QUrl(ApiConfig::baseUrl + "/api/blog/comments"));
    req.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    if (!token.isEmpty()) req.setRawHeader("Authorization", token.toUtf8());
    QJsonObject j; j["post_id"] = postId; j["content"] = content;
    QNetworkReply* reply = m_manager->post(req, QJsonDocument(j).toJson());
    connect(reply, &QNetworkReply::finished, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            emit commentAdded();
