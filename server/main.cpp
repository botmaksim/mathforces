...
bool initDatabase() {
  ...
  QSqlQuery qq;
  qq.exec("CREATE TABLE IF NOT EXISTS friends (id SERIAL PRIMARY KEY, user_id "
          "INTEGER REFERENCES users(id) ON DELETE CASCADE, friend_id INTEGER "
          "REFERENCES users(id) ON DELETE CASCADE, created_at TIMESTAMP "
          "DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id, friend_id));");
