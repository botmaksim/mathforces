...
class Database {
public:
  static bool init(const QString &dbName, const QString &user,
                   const QString &pass, const QString &host = "127.0.0.1",
                   int port = 5432);
  static void createInitialUsers();

  static QString getThreadLocalConnection();

  static QJsonObject authenticate(const QString &username,
                                  const QString &password);
  static QJsonObject authenticateByEmail(const QString &email,
                                         const QString &password);
  static QJsonObject authenticateOAuth(const QString &email,
                                       const QString &googleId,
                                       const QString &name);
  static bool registerByEmail(const QString &email, const QString &password,
  ...
  static bool isVirtualParticipating(int contestId, int userId,
  ...
  static QJsonArray getArchiveTasks(const QString &tags, int minDiff,
                                    int maxDiff);
  static QJsonObject getArchiveTaskDetails(int taskId);
  static QJsonArray getResults(int contestId);

  static int savePendingSubmission(int taskId, int userId,
                                   const QString &answer, bool isUpsolving);
  static void updateSubmissionResult(int submissionId, int score,
  ...
  static void updateHackStatus(int hackId, bool isSuccessful,
                               const QString &explanation);
  static QJsonObject getHackContext(int hackId);

  static int createContestInitial(int authorId);
  static bool updateContest(int contestId, int authorId, const QString &title,
                            const QString &description, const QString &start,
                            float durationHours, bool isPublished);
  static bool createTask(int contestId, const QString &type,
                         const QString &title, const QString &description,
                         int maxScore, int maxSubmissions,
                         const QString &correctAnswer, const QString &editorial,
                         const QString &aiComment, bool sendEditorial,
                         const QString &tags, int difficulty);
  ...
  // Social / Blog
  static QJsonObject getUserProfile(int userId, int requestorId,
