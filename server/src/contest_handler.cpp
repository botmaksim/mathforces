...
namespace {
QString typstExecutablePath() {
  const QString envPath = qEnvironmentVariable("MATHFORCES_TYPST_BIN");
  if (!envPath.isEmpty() && QFileInfo::exists(envPath))
    return envPath;

  const QString appDir = QCoreApplication::applicationDirPath();
  const QString root = QDir(appDir).absoluteFilePath("..");
#ifdef Q_OS_WIN
  const QString exe = "typst.exe";
#else
  const QString exe = "typst";
#endif

  const QStringList candidates = {
      QDir(appDir).absoluteFilePath("tools/typst/" + exe),
      QDir(root).absoluteFilePath("tools/typst/" + exe),
      QDir(root).absoluteFilePath("../tools/typst/" + exe),
      QDir::current().absoluteFilePath("tools/typst/" + exe),
      QDir::current().absoluteFilePath("../tools/typst/" + exe)};
  ...
  const QString systemTypst = QStandardPaths::findExecutable(exe);
  if (!systemTypst.isEmpty())
    return systemTypst;
...
void ContestHandler::registerRoutes(QHttpServer &server) {
  ...
  server.route("/api/archive/task", QHttpServerRequest::Method::Get,
               [](const QHttpServerRequest &req) {
                 int taskId = req.query().queryItemValue("id").toInt();
                 QJsonObject task = Database::getArchiveTaskDetails(taskId);
                 if (task.isEmpty()) {
                   return QHttpServerResponse(
                       QJsonObject{{"error", "Archive task not found"}},
                       QHttpServerResponder::StatusCode::NotFound);
                 }
                 return QHttpServerResponse(task);
               });
  ...
  server.route(
      ...
      [](const QHttpServerRequest &req) {
        ...
        const QString typstPath = typstExecutablePath();
        if (typstPath.isEmpty()) {
          QFile::remove(tempInPath);
          QFile::remove(outPath);
          return QHttpServerResponse(
              QByteArray("Local Typst compiler not found. Put the official typst binary into tools/typst or set MATHFORCES_TYPST_BIN."),
              QHttpServerResponder::StatusCode::InternalServerError);
        }

        QProcess process;
        process.start(typstPath, QStringList() << "compile" << tempInPath << outPath);
        if (!process.waitForFinished(30000)) {
          ...
          QFile::remove(outPath);
          return QHttpServerResponse(
              QByteArray("Typst compilation timeout"),
              QHttpServerResponder::StatusCode::BadRequest);
        }

        if (process.exitCode() != 0 ||
            process.exitStatus() != QProcess::NormalExit) {
          QString errlog = QString::fromUtf8(process.readAllStandardError());
